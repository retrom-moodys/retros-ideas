...
DATA_DIR="/opt/ec2-web-app/data"

echo "==> Installing system packages"
sudo dnf update -y
sudo dnf install -y nginx python3.11 python3.11-pip

echo "==> Creating app directories"
sudo mkdir -p "$APP_ROOT" "$WEB_ROOT" "$DATA_DIR"
sudo chown -R ec2-user:ec2-user "$APP_ROOT" "$DATA_DIR"

echo "==> Copy project files into place (run from repo root on EC2)"
echo "    Expected layout:"
echo "      $APP_ROOT/backend"
echo "      $WEB_ROOT/index.html"
echo "      $DATA_DIR/submissions.db (created on first API request)"
...
if [ ! -f "$APP_ROOT/backend/.env" ]; then
  echo "==> Creating default backend/.env (SQLite, email disabled)"
